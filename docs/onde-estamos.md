...
- **EP-07 — Governança**: auditoria (PLT-05) ✅ + notificações (PLT-07) ✅ — sino no shell, canal do usuário sobre o SSE + **push nativo** ✅ (Web Push com VAPID, `0066`): avisa com o console fechado, payload SEM conteúdo da conversa (o serviço de push é de terceiro e a notificação aparece na tela de bloqueio), assinatura morta (404/410) é removida, e a permissão é pedida no CLIQUE — nunca no carregamento.
- **EP-03 — Saúde da frota** ✅: entrega 24h + alertas + risco de banimento na tela "Meus Números". Tier/pagamento/qualidade são do oficial (Meta), entram com ele. **Pausa automática por queda** ✅ — e no caminho descobriu-se que o botão de pausa manual não pausava nada (o gateway não lia a coluna). Pausa vale só para envio programático; resposta de gente continua saindo. Sem retomada automática: com o disparo parado não nascem amostras para observar a recuperação.
